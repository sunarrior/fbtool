...
def get_post_on_feed():
    ...
    with codecs.open('./data/new_feed.json', 'w', 'utf-8') as file:
        json.dump(lst_post_id, file, indent=4)
        file.close()
    return lst_post_id
...
def get_reaction_count(post_id, type):
    ...
    with codecs.open('./data/reaction_count.json', 'w') as file:
        json.dump(reaction_count_json, file, indent=4)
        file.close()
...
def get_albums(option, opt_str, value, parser):
    ...
    with codecs.open("./data/album.json", "w+", "utf-8") as outfile:
...
def get_pages_like(option, opt_str, value, parser):
    ...
    with codecs.open("./data/pages_liked.json", "w", "utf-8") as outfile:
...
def get_music_like(option, opt_str, value, parser):
    ...
    musics_liked = {
        'quantity':str(len(data['music']['data'])),
        'info': arr
    }
    with codecs.open("./data/musics_liked.json", "w+", "utf-8") as outfile:
        json.dump(musics_liked, outfile, indent=4)
        outfile.close()
    print(f"Musics liked: {musics_liked}")

def get_info_user(option, opt_str, value, parser):
    PARAMS_NAME = {'fields':'name', 'access_token':token}
    PARAMS_BIRTHDAY = {'fields':'birthday', 'access_token':token}
    PARAMS_EMAIL = {'fields':'email', 'access_token':token}
    PARAMS_LOCATION = {'fields':'location', 'access_token':token}
    PARAMS_GENDER = {'fields':'gender', 'access_token':token}
    ...
    with codecs.open('./data/basic_info.json', 'w', 'utf-8') as file:
...
def get_favorite_teams(option, opt_str, value, parser):
    PARAMS_FAVORITE = {'fields':'favorite_teams', 'access_token':token}
    user_favorite = requests.get(url = URL + '/me', params = PARAMS_FAVORITE).json()
    favorite_teams_json = {
        "quantity": len(user_favorite['favorite_teams']),
        "team_name": []
    }
    print(f"Get list favorite user:")
    for i in range(len(user_favorite['favorite_teams'])):
        favorite_teams_json['team_name'].append(user_favorite['favorite_teams'][i]['name'])
        print(user_favorite['favorite_teams'][i]['name'])

    with codecs.open('./data/favorite_teams.json', 'w', 'utf-8-sig') as file:
        json.dump(favorite_teams_json, file, indent=4)
        file.close()
...
def main():
    ...
    parser.add_option("-t", help="set type of reaction [Like, Love, Care, Haha, Wow, Sad, Angry] \
    ...
    parser.add_option("--favteams", action="callback", callback=get_favorite_teams,
                        help="show favorite")
